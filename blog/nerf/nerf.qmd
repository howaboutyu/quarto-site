...
$$
w_X(x) = \frac{T_x(1-\exp(-\sigma_x \delta_x))}{k} \quad 1 < x < N_c 
$$

where $k=\frac{w_x}{\sum_j^{N_c} w_j }$ is the normalizing factor so the area under the PDF is 1.
...
fig = px.line(x=np.arange(1, len(weights) + 1), y=weights, title="sample figure")
fig.show()
