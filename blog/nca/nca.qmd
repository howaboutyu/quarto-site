...
::: {#fig-channel-prop layout-ncol=4 }
![c1](channel_0.gif)
...
The above shows the evolution of the CA for each channel. 16 channels are shown.
:::
...
# apply
kernel_x, kernel_y = nca.create_perception_kernel(input_size=16, output_size=16)

perceived_data_jnp = nca.perceive(data, kernel_x, kernel_y) 
perceived_data = np.copy(perceived_data_jnp)
perceived_data = np.transpose(perceived_data, (0, 2, 3, 1))

perception_x = perceived_data[:, :, :, :16]

data = perception_x

normalized_data = np.zeros_like(data)

# normalize from 0-1, per channel, using log scale
for channel in range(data.shape[-1]):
    log_data = np.log10(data[:, :, :, channel] - data.min() + 1)  # add 1 to avoid log(0)
    normalized_data[:, :, :, channel] = (log_data - np.min(log_data)) / (np.max(log_data) - np.min(log_data))
# nan to 0
...
# Create a GIF animation for each channel
for channel in range(data.shape[-1]):
    frames = []
    for t in range(data.shape[0]):
        img = normalized_data[t, :, :, channel]
        ...
        # Resize the image
        colored_img = colored_img.resize((colored_img.width * resize_factor, colored_img.height * resize_factor), Image.NEAREST)
        frames.append(colored_img)

    # Save the GIF animation
    imageio.mimsave(f'channel_{channel}.gif', frames, format='GIF', duration=0.2)
